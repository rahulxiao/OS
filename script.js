...
const qaData = {
    ...
    critical: [
		...
		,
		{
			question: "In critical-section problem",
			answer: "Q1. What is a race condition?\n\nAnswer:\nA race condition is an undesirable situation that occurs when two or more processes (or devices) try to perform operations at the same time on a shared resource, but the correctness depends on the order of execution. If the proper sequence is not maintained, the result becomes inconsistent or incorrect.\n\nQ2. What is the Critical Section Problem?\n\nAnswer:\nIn a system with multiple processes {p0, p1, …, pn-1}, each process has a critical section, where it accesses or modifies shared resources (variables, tables, files, etc.).\nThe Critical Section Problem is to design a protocol that ensures:\n\nWhen one process is executing in its critical section, no other process can execute in its critical section.\n\nEach process must request permission before entering (entry section), and after leaving (exit section), it continues in its remainder section.\n\nQ3. What are the three requirements of a correct solution to the Critical Section Problem?\n\nAnswer:\n\nMutual Exclusion:\nIf one process Pi is in the critical section, no other process may enter its critical section.\n\nProgress:\nIf no process is in the critical section and there are processes that want to enter, one of them must be allowed to proceed. The decision cannot be postponed indefinitely.\n\nBounded Waiting:\nThere must be a limit on how many times other processes can enter their critical section after a process has made a request and before that request is granted.\n\n(Also assume each process runs at nonzero speed, with no assumption about relative speeds of processes.)\n\nQ4. Explain Peterson’s Solution for two processes.\n\nAnswer:\nPeterson’s Solution provides software synchronization for two processes. It assumes atomic load and store operations.\n\nShared variables:\n\nint turn;         // whose turn to enter critical section\nboolean flag[2];  // flag[i] = true if process Pi wants to enter\n\n\nWorking principle:\n\nEach process sets its flag to true (wants to enter).\n\nIt then gives the turn to the other process.\n\nA process enters the critical section only if either:\n\nThe other process is not interested (flag[j] = false), OR\n\nIt’s this process’s turn.\n\nLimitations:\n\nAlgorithmically correct, but not guaranteed on modern multiprocessor architectures due to compiler/hardware instruction reordering.\n\nStill important as a conceptual solution to demonstrate solving the critical section problem."
		}
    ],
    security: [
        {
